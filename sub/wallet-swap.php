...
/*
	if ( $("#select_swap_from").val() == "clwmc" ) {
		...
		can_swap_sn3 = parseInt((token.balance - token.gas_fee ) / 3600);
		$("#swap_to").val(can_swap_sn3);
		$("#swap_from").val(can_swap_sn3 * 3600 + token.gas_fee);

		if ( can_swap_sn3 < 1000) {
	...
	if ( $("#select_swap_from").val() == "mm" ) { 
		...
		can_swap_sn3 = parseInt((token.balance - token.gas_fee ));
		$("#swap_to").val(addCommas(can_swap_sn3));
...
//	console.log($("#swap_to").val());
	...
	var SN3_value = $("#swap_to").val().replace(/[^0-9.]/g, '');
	$("#swap_to").val(addCommas(SN3_value.replace(/^[ 0]/g,'')));
			...
			switch(args.trim()){
			 case("done"):
				...
				$.ajax({
				   type:"POST",        
				   url:"/includes/proc_baviWallet.php",     
				   data : ({mode:"bat_swap", swap_from:"bat", swap_from_amount:$("#swap_from").val().replace(/[^0-9.]/g, ''), swap_to:"sn3", swap_to_amount:$("#swap_to").val().replace(/[^0-9.]/g, ''), swap_gas_fee:BAT.gas_fee, game_code: $("#select_swap_from").val() }),
				   timeout : 600000,
				   cache : false,
				   success: function whenSuccess(args){
